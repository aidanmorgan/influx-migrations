...
public class ExecuteResultResolver : AbstractResolverFunction
{
    ...
    internal static StringResolvable ResultValue(IResolvable<string?> key, string originalString,
        Func<IOperationExecutionContext, object?> getter)
    {
        return new StringResolvable(ResolutionType.Result, originalString, (x) =>
            {
                ...
                var property = Enumerable.FirstOrDefault<PropertyInfo?>(resultType.GetProperties(),
                    x => string.Equals(x?.Name, val, StringComparison.InvariantCultureIgnoreCase), null);
                if (property == null)
                {
                    throw new MigrationResolutionException(
                        $"Cannot find an appropriate property on {resultType.FullName} for variable {val}");
                }

                if (!property.CanRead)
                {
                    throw new MigrationResolutionException(
                        $"Cannot find readable property for variable {val} on {resultType.FullName}");
                }

                if (property.PropertyType != typeof(string))
                {
                    throw new MigrationResolutionException(
                        $"Property for value {val} on {resultType.FullName} is not a string.");
                }
...
public class CommitResultResolver : AbstractResolverFunction
{
    ...
    {
        var keyResolvable = next(Unwrap(entry, Prefix));
        return keyResolvable == null
            ? null
            : ExecuteResultResolver.ResultValue(keyResolvable, entry, x => x.CommitResult);
    }
...
public class RollbackResultResolver : AbstractResolverFunction
{
    public override IResolvable<string?> Parse(string entry, Func<string, IResolvable<string?>> next)
    {
        ...
        return keyResolvable == null
            ? null
            : ExecuteResultResolver.ResultValue(keyResolvable, entry, x => x.RollbackResult);
    }
